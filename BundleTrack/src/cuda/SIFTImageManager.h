...
public:
	...
	void getCurrMatchKeyPointIndicesDEBUG(std::vector<uint2>& keyPointIndices, std::vector<unsigned int>& numMatches, bool filtered) const
	{
		...
		if (filtered)	{ cudaMemcpy(numMatches.data(), d_currNumFilteredMatchesPerImagePair, sizeof(unsigned int)*numMatches.size(), cudaMemcpyDeviceToHost); }
		else			{ cudaMemcpy(numMatches.data(), d_currNumMatchesPerImagePair, sizeof(unsigned int)*numMatches.size(), cudaMemcpyDeviceToHost); }
		if (filtered)	{ keyPointIndices.resize(numMatches.size() * MAX_MATCHES_PER_IMAGE_PAIR_FILTERED); cudaMemcpy(keyPointIndices.data(), d_currFilteredMatchKeyPointIndices, sizeof(uint2) * keyPointIndices.size(), cudaMemcpyDeviceToHost); }
		else			{ keyPointIndices.resize(numMatches.size() * MAX_MATCHES_PER_IMAGE_PAIR_RAW); cudaMemcpy(keyPointIndices.data(), d_currMatchKeyPointIndices, sizeof(uint2) * keyPointIndices.size(), cudaMemcpyDeviceToHost); }
	}
